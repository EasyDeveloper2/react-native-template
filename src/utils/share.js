...
const shareNewsToTimeline=({webpageUrl='',thumbImage,description,title})=>{
 ...
 const shareMultiImagesToWx= async ({urls,title})=>{
    //  let data  = await getBase64ByUrl(urls[0])
    //  data = `data:image/png;base64,` + data
    //  let t = await saveLocal(urls[0])
    // let photoAsset = await CameraRoll.getPhotos({first:5,assetType:'Photos'}) 
   
    //  let photos = []
    //  for(let item of photoAsset.edges){
    //      console.log(item.node.image)
    //      photos.push(item.node.image.uri)
    //  }
